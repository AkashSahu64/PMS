...
const PackagesList = ({ patientId }) => {
  ...
  const updateTreatmentMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries(["packages", patientId]); // to update completed count
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Status:{" "}
                  <span
                    className={`capitalize ${pkg.status === "active" ? "text-green-600" : "text-gray-600"}`}
                  >
                    {pkg.status}
                  </span>
                ...
                >
                  <div className="p-4 space-y-3">
                    {treatmentsData[pkg._id]?.map((treatment) => {
                      const isEditing =
                        !!editingTreatments[pkg._id]?.[treatment._id];
                      const editedValues =
                        editingTreatments[pkg._id]?.[treatment._id] || {};
                        ...
                        >
                          <div className="flex justify-between items-center mb-2">
                            ...
                            {(() => {
                              ...
                              if (treatment.attended) {
                                statusText = "Completed";
                                statusClass = "bg-green-100 text-green-800";
                              } else if (treatmentDate > today) {
                                statusText = "Pending";
                                statusClass = "bg-yellow-100 text-yellow-800";
                              } else if (
                                treatmentDate.getTime() === today.getTime()
                              ) {
                                statusText = "Active";
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div>
                              ...
                              <input
                                type="date"
                                value={
                                  new Date(editedValues.date ?? treatment.date)
                                    .toISOString()
                                    .split("T")[0]
                                }
                                onChange={(e) =>
                            ...
                            <div className="flex items-center">
                              ...
                              <input
                                type="checkbox"
                                checked={
                                  editedValues.attended ?? treatment.attended
                                }
                                onChange={(e) =>
                                  handleTreatmentChange(
                                    pkg._id,
                          ...
                          {isEditing && (
                            <div className="mt-2 flex justify-end">
                              <button
                                onClick={() =>
                                  handleSaveTreatment(pkg._id, treatment)
                                }
